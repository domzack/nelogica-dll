...
def getAllTickersByStock():
    global profit_dll
    print('GetAllTicker - Retorno via AssetListCallback, AssetListInfoCallback e AssetListInfoCallbackV2')
    print('---------------------------------------------------------------------------------------------')
    stock = input('Bolsa: ')
    
    if (stock == ''):
        res = profit_dll.GetAllTicker('') #retorna todos os ativos de Bovespa e BMF
    else:
        res = profit_dll.GetAllTicker(stock) #retorna todos os ativos de uma bolsa específica: B ou F 
